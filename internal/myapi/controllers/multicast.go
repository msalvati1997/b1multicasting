...
func (c *MulticastController) PutMessage() {
	...
	myapi.GMu.RLock()
	defer myapi.GMu.RUnlock()

	group, ok := myapi.MulticastGroups[multicastId]
	if !ok {
...
func (c *RegistryController) CreateGroup() {
	...
	register, err := myapi.RegistryClient.Register(context.Background(), &proto.Rinfo{
		MulticastId: multicastId,
		ClientPort:  uint32(myapi.GrpcPort),
	})
	if err != nil {
	...
	members := make(map[string]myapi.Member, 0)

	for memberId, member := range register.GroupInfo.Members {
		members[memberId] = myapi.Member{
			MemberId: member.Id,
	...
	group = &myapi.MulticastGroup{
		ClientId: register.ClientId,
		Group: &myapi.MulticastInfo{
	...
	myapi.MulticastGroups[register.GroupInfo.MulticastId] = group
...
func (c *RegistryController) InitGroup(info *proto.MGroup, group *myapi.MulticastGroup, b bool) {
	...
	groupInfo, err = myapi.RegistryClient.Ready(context.Background(), &proto.RequestData{
...
func (c *RegistryController) StartGroup() {
	...
	_, err := myapi.RegistryClient.StartGroup(context.Background(), &proto.RequestData{
...
func updateMulticastGroup(groupInfo *proto.MGroup, multicastGroup *myapi.MulticastGroup) {
...
func waitStatusChange(groupInfo *proto.MGroup, multicastGroup *myapi.MulticastGroup, status proto.Status) (*proto.MGroup, error) {
	var err error

	for groupInfo.Status == status {
		time.Sleep(time.Second * 5)
		groupInfo, err = myapi.RegistryClient.GetStatus(context.Background(), &proto.MulticastId{MulticastId: groupInfo.MulticastId})
		if err != nil {
			return nil, err
		}
